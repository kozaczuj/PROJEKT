...
{
  int opt;
  char *inp = NULL;	// nazwa pliku zawierającego punkty
  char *out = NULL;
  char *gpt = NULL;
  double fromX = 0;
  double toX = 0;
  ...
  pts.n = 0;	// zeruję licznik punktów
  spl.n = 0;
  ...
  if (inp != NULL) {
    ...
    FILE *inf = fopen (inp, "r");	// inf zawiera punkty (inp - nazwa)
    if (inf == NULL) {
      fprintf (stderr, "%s: can not read points file: %s\n\n", argv[0], inp);
      exit (EXIT_FAILURE);
    }

    if (read_pts_failed (inf, &pts)) {	// odczytuje punkty z inf do struktury pts
    ...
    ouf = fopen (out, "w");	// plik do którego wypiszemy spline
    if (ouf == NULL) {
      fprintf (stderr, "%s: can not write spline file: %s\n\n", argv[0], out);
      exit (EXIT_FAILURE);
    }

    make_spl (&pts, &spl);	// robimy spline

    if( spl.n > 0 )
			write_spl (&spl, ouf);	// wypisujemy spline do ouf
