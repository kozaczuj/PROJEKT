...
make_spl(points_t * pts, spline_t * spl)
{
	...
	matrix_t       *eqs= NULL;	// tworzę macierz
	double         *x = pts->x;	// przepisuje współ. x
	double         *y = pts->y;	// przepisuje współ. y
	double		a = x[0];	// a równe x0
	double		b = x[pts->n - 1];	// b = ostatnia wartość x
	int		i, j, k;
  ...
  char *nbEnv= getenv( "APPROX_BASE_SIZE" );
	...
	eqs = make_matrix(nb, nb + 1);	// alokuje miejsce na macierz i wypełnia zerami
...
#endif
	if (G == 0) {
		if (piv_ge_solver(eqs)) {	// wykorzystuje gausa do rozwiazania macierzy eqs
			spl->n = 0;
			return;
		}
	}
	if (G == 1) {
		if (piv_gs_solver(eqs)) {
			spl->n = 0;
			return;
		}
	}	
